...
public class CryptoService {
    ...
    public CryptoService(Settings settings) {
        ...
        try(SecureString key = EntSearchField.ENCRYPTION_KEY_SETTING.get(settings)) {
            if (key == null || key.isEmpty()) {
                throw new ElasticsearchException("setting [" + EntSearchField.ENCRYPTION_KEY_SETTING.getKey() + "] must be set in keystore");
            }
            SecretKey systemKey = readSystemKey(key);
    ...
    private static SecretKey readSystemKey(SecureString key) {
        return new SecretKeySpec(key.toString().getBytes(StandardCharsets.UTF_8), KEY_ALGO);
    }
